...
// Author: Mahesh Acharekar  
// Date:   8/22/1999
// Modified Date: 11/30/1999
// Modified By:   Imtiyaz Loya ("mm/dd/yyyy")
...
topBackground    = "lightgrey";         //#ffc86c #d7eef3 BG COLOR OF THE TOP FRAME
bottomBackground = "white";         // #e8f4f8 BG COLOR OF THE BOTTOM FRAME
tableBGColor     = "lightgrey";         //#edfdfd #e7f7f7 BG COLOR OF THE BOTTOM FRAME'S TABLE
parenttableColor = "lightgrey";		//#003599 #c1cdd3
cellColor        = "white";     // e8f4f8 TABLE CELL BG COLOR OF THE DATE CELLS IN THE BOTTOM FRAME
focusCellColor   = "#C4E1FF";
headingCellColor = "#003599";         // #d7eef3 TABLE CELL BG COLOR OF THE WEEKDAY ABBREVIATIONS
headingTextColor = "#ffff00";         // #427b94 TEXT COLOR OF THE WEEKDAY ABBREVIATIONS

dateColor        = "#000000";          // TEXT COLOR OF THE LISTED DATES (1-28+)
focusColor       = "#0000ff";       // TEXT COLOR OF THE SELECTED DATE (OR CURRENT DATE)
hoverColor       = "#ff0000";       // TEXT COLOR OF A LINK WHEN YOU HOVER OVER IT
fontStyle        = "10pt verdana";           // TEXT STYLE FOR DATES
headingFontStyle = "bold 9pt verdana";      // TEXT STYLE FOR WEEKDAY ABBREVIATIONS

tablebordercolor = "#000000"
fieldstyle = "font-family: Verdana;font-size: 8pt;font-style: normal;font-weight: normal;color: #000000;text-decoration: none;background-color: #FFFFFF;border: #000000;border-style: solid;border-top-width: 1px;border-right-width: 1px;border-bottom-width: 1px;border-left-width: 1px;height: 17px;text-align: left;vertical-align: middle;padding-top: 0px;padding-bottom: 0px"
...
//function to get date in mm/dd/yy format from date which is passed as a string in dd/mm/yy format
	function getmdy(txt){
...
function showCalendar(dateField,TopVal,LeftVal) {
	//Pass string for pop-up window imtiyaz 10-11-2000
	var Prefs
	//setting event handler for the datefield
	if(arguments[3] == null)
		calEvent="";
	else
		calEvent=arguments[3];
		
	Prefs = "resizable=no,dependent=yes,titlebar=yes,width=230,height=230,left=" + LeftVal + ",top=" + TopVal
	winPrefs=Prefs;
    ...
    calDocFrameset = calDocTop + calDocBottom
    /*calDocFrameset = 
        "<HTML><HEAD><TITLE>BPCL Calendar</TITLE></HEAD>\n" 
        +
...
function buildTopCalFrame() {
    ...
    // CREATE THE TOP FRAME OF THE CALENDAR "<CENTER>" +
    var calDoc =
        "<HTML>" +
        "<HEAD>" +
        "</HEAD>" +
        "<BODY BGCOLOR='" + topBackground + "' TOPMARGIN = '10' MARGINWIDTH = '0' MARGINHEIGHT = '0'>" +
        "<FORM NAME='calControl' onSubmit='return false;'>" +
        "<TABLE ALIGN='CENTER' CELLPADDING=0 CELLSPACING=1 BORDER=0>" +
        "<TR><TD COLSPAN=7 align='center'>" +
        getMonthSelect() +
        //"<INPUT id='year' NAME='year' VALUE='" + calDate.getFullYear() + "'TYPE=TEXT SIZE=4 MAXLENGTH=4 onChange='parent.opener.setYear()' Disabled>" +
        "<INPUT id='year' NAME='year' VALUE='" + iYear + "'TYPE=TEXT SIZE=4 MAXLENGTH=4 onChange='parent.opener.setYear()' Disabled>" +
        "</TD>" +
        "</TR>" +
        "<TR>" +
        "<TD COLSPAN=7>" +
        ...
        "TYPE=BUTTON NAME='clear' VALUE='Clear' onClick='parent.opener.ClearDate()'><INPUT " +
...
function buildBottomCalFrame() {       
	...
	var paddingChar = "&nbsp;";
        ...
        if (currentDay == day) {
            dayType = "focusDay";
            activeCellColor = focusCellColor
        }
        else {
            dayType = "weekDay";
        ...
        calDoc += "<TD align=center bgcolor='" + activeCellColor + "'>" +
...
function writeCalendar() {
    ...
    */
    ...
    top.newWin.document.write(calDocTop + calDocBottom);
...
function setToday() {
	...
	top.newWin.document.calControl.month.selectedIndex = iMth;

    // SET YEAR VALUE
    //top.newWin.frames['topCalFrame'].document.calControl.year.value = year;
	top.newWin.document.calControl.year.value = iYear;
...
function setYear() {
    ...
    if (isFourDigitYear(iYear)) {
        calDate.setFullYear(iYear);
        writeCalendar();
    }
    else {
...
function setPreviousYear() {
	...
	var year  = top.newWin.document.calControl.year.value;
	iYear = year;
    if (isFourDigitYear(iYear) && iYear > 1000) {
        iYear--;
        calDate.setFullYear(iYear);
        //top.newWin.frames['topCalFrame'].document.calControl.year.value = year;
        top.newWin.document.calControl.year.value = iYear;
...
function getMonthSelect() {
    ...
    var activeMonth = iMth;//calDate.getMonth();

    // START HTML SELECT LIST ELEMENT
    monthSelect = "<SELECT id='month' NAME='month' onChange='parent.opener.setCurrentMonth()'>";
...
function buildCalParts() {
    ...
    calendarBegin =
        ...
        "</STYLE>" +
        "</HEAD>" +
        "<BODY BGCOLOR='" + bottomBackground + "' LEFTMARGIN = '1' TOPMARGIN = '6' MARGINWIDTH = '0' MARGINHEIGHT = '0'>" +
        "<FORM name='calDisp' onSubmit='return false;'>" +
        "<Div id='oDiv'>" +
		"<TABLE CELLPADDING=0 CELLSPACING=0 BORDER=0 ALIGN=CENTER>" +
        "<TR>" +
        "<TD BGCOLOR = " + parenttableColor + ">";
        
        // NAVIGATOR NEEDS A TABLE CONTAINER TO DISPLAY THE TABLE OUTLINES PROPERLY
        //if (isNav) {
        //    calendarBegin += 
        //        "<TABLE CELLPADDING=0 CELLSPACING=0 BORDER=" + tableBorder + " BORDERCOLOR=" + tablebordercolor + " ALIGN=CENTER BGCOLOR='" + tableBGColor + "'><TR><TD>";
        //}

        // BUILD WEEKDAY HEADINGS
        calendarBegin +=
            "<TABLE CELLPADDING=1 CELLSPACING=1 BORDER=" + tableBorder + " BORDERCOLOR=" + tablebordercolor + " ALIGN=CENTER>" +
            weekdays +
            "<TR>";
...
function returnDate(inDay)
{
    ...
    if (calDateField.name == 'txtleavefromdate' || calDateField.name == 'txtleavetodate')
    {
	totaldays()	
    }
